...
from PyQt5.QtCore import Qt, pyqtSlot, pyqtSignal, QRect, QRectF, QSize, QPoint, QPointF
...
from imagedrawer import mergeImages, genQImage
...
class Canvas(widgets.QWidget):
	def __init__(self, parent=None):
		...
		self.background_color = QColor('#ff0000')
...
#		self.setMouseTracking(True)
	...
	def initImage(self):
		...
		w,h = min(self.width(), 1), min(self.height(),1)
		self.cvImg = self._genEmptyCvImg( (h,w, 4) )

	def _genEmptyCvImg(self, shape):
	...
	def _resize_hlpr(self):
		if not self.img.isNull():
			self.img = resizeImage(self.img, self.size(), self.background_color)
		
		self._resizeCvImg() 
	
	def _resizeCvImg(self):
		"""
		resizing self.cvImg: only make it bigger, never smaller!
		(smaller would result in loss of data!)
		"""
		cvheight, cvwidth = self.cvImg.shape[:2]
		newwidth, newheight = self.width(), self.height()
		if newwidth > cvwidth or newheight > cvheight:
			tarwidth = max(cvwidth, newwidth)
			tarheight = max(cvheight, newheight)

			newcvimg = self._genEmptyCvImg( (tarheight, tarwidth, 4) )
			newcvimg[:self.cvImg.shape[0], :self.cvImg.shape[1]] = self.cvImg
			self.cvImg = newcvimg
	...
	def _clearImg(self):
		...
		painter.end()
		del painter
		self.cvImg = self._genEmptyCvImg( self.cvImg.shape )
	...
	def mouseReleaseEvent(self, e):				
		...
		if pnts_cnt > 2:
			...
			with open('points', 'wb') as fos:
				np.save(fos, avg.getPoints() )
		else:
	...
	def drawLine(self, p0, p1, diameter=2., col=None, alpha=1):
		if not col:
			col = self.pen_color
		...
		qp0 = QPoint(p0[0], p0[1])
		qp1 = QPoint(p1[0], p1[1])
		
		self.painter.drawLine(qp0, qp1)
...
#		self.scene.addItem(lne)
	...
	def _getCvImgSize(self):
		cvwidth  = self.cvImg.shape[1]
		cvheight = self.cvImg.shape[0]
		return (cvwidth, cvheight)
	...
	def drawLines(self, pnts):	
		...
		cvImgWidht, cvImgHeight = self._getCvImgSize()
		lineDrawer = LineDrawer(pnts)
		linesImg = lineDrawer.drawLines(cvImgWidht, cvImgHeight, pen_width, col)
		
		self.cvImg = mergeImages(self.cvImg, linesImg)

		qlinesimg = genQImage(self.cvImg)
		self.img = qlinesimg
...
#			p0 = [x1,y1]
#			p1 = [x2,y2]
#			self.drawLine(p0, p1, pen_width, col)#, stroke_width=pen_width, col=col)
